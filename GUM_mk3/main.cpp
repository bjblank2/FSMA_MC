...
int main(void) {
	...
	fillRuleList(mc_rules, "Aust_Rules.txt", 0); // Populate mc_rules. Rules are defined as follows, [species (0,1,2 for Ni,Mn,In)]:[NA if monomer,dist of dymer, AB,AC,BC if trimer]:no-spin(0) or spin(1): value
	int shape[3] = { 4, 4, 4 }; // The dementions of the simulation cell in terms of the unit cell
	vector<int> species{ 512, 256, 256 }; // Number of Ni, Mn, In atoms respectivley 
	...
	sim_cell.initSimCell(input_file, dist_list, shape, species, 1, string("DEFAULT"), string("AUST"), string("RAND"), string("ORDERED"),use_poscar); // Create and initalize the simulation cell
  	cout << "Testing" << '\n';
	cout << "begining MC" << '\n';
	...
	runMetropolis3(100, 1500, 10, -10, sim_cell, mc_rules); // Run MC using implementation number 3 (The fast one that assumes only spin flips)
	...
	//runMetropolis5(100 , 1500, 1, -10, sim_cell, mc_rules); // Run MC using implementaion number 5 (Always use spin and species filps)
